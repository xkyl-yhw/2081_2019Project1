...
public class TestTriangle : MonoBehaviour
{
    ...
    private void OnDrawGizmos()//在Gizmos中绘制可被点选的物体
    {
    ...
    {
        return transform.TransformPoint(src);//转为世界坐标
    }
    ...
    private class TriangleMeshCreator
    {
        ...
        private Mesh Create(float sideLength, float angleDegree)
        {
            ...
            float sinA = Mathf.Sin(angleDegree);
        ...
        {
            return (int)((sideLength - this._sideLength) * SIDE_LENGTH_PRECISION) != 0 || (int)((angleDegree - this._angleDegree) * ANGLE_DEGREE_PRECISION) != 0;
        }
    }
